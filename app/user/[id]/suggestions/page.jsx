  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.log(userInfo.suggestions_to_me);
      } catch (error) {
          ...
          <>
            <div className="heading-one text-gray-50 flex items-center gap-2 w-full">
              ...
              <div className="flex flex-col">
                ...
                <div> was suggested to watch</div>
              </div>
            </div>
            <div className="pt-4 grid  sm:grid-cols-3 md:grid-cols-6 lg:grid-cols-9  xl:grid-cols-12 gap-6">
              {userInfo.suggestions_to_me.length ? (
                <>
                  {userInfo.suggestions_to_me.map((item) => {
                    return (
                      <Suggestion
                        item={item}
                        isUserPage={isUserPage}
                        toMe={item.proposed_to.id === userInfo?.id}
                      ></Suggestion>
                    );
                  })}
                </>
